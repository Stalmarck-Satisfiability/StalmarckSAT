...
mod tests {
    ...
    fn test_empty_file() {
        ...
        assert_eq!(formula.num_clauses(), 0, "Empty file should have zero clauses");
    }
    ...
    fn test_comments_only() {
        ...
        let result = parser.parse_dimacs(file.path());
        assert!(result.is_ok(), "Should not error on file with only comments");
        
        let formula = result.unwrap();
        assert_eq!(formula.num_clauses(), 0, "File with only comments should have zero clauses");
    }
    ...
    fn test_simple_cnf() {
        ...
        assert_eq!(clauses[0], vec![1, 2, -3], "First clause should be [1, 2, -3]");
        assert_eq!(clauses[1], vec![-1, 2, 3], "Second clause should be [-1, 2, 3]");
    }
    ...
    fn test_complicated_cnf() {
        ...
        let clauses = formula.get_clauses();
        assert_eq!(clauses[0], vec![1, -2, 3], "First clause should be [1, -2, 3]");
        assert_eq!(clauses[1], vec![-1, 2, -3, 4], "Second clause should be [-1, 2, -3, 4]");
    ...
    fn test_invalid_literal() {
        ...
        assert!(parser.get_error().contains("Invalid literal"), "Error message should mention invalid literal");
    }
    ...
    fn test_nonexistent_file() {
        ...
        assert!(parser.has_error(), "Parser should have error flag set");
        assert!(parser.get_error().contains("Failed to open"), "Error message should mention failed to open");
    }
}
