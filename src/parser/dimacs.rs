        ...
        for (line_num, line_result) in reader.lines().enumerate() {
                ...
                if tokens.len() >= 4 && tokens[1] == "cnf" {
                    if let Ok(num_vars) = tokens[2].parse::<usize>() {
                        ...
                        if let Ok(num_clauses) = tokens[3].parse::<usize>() {
                            ...
                            self.error_message = format!("Invalid number of clauses in line {}", line_num + 1);
                            self.has_error_flag = true;
                            return Err(crate::Error::Parse(self.error_message.clone()));
                        }
                    } else {
                        self.error_message = format!("Invalid number of variables in line {}", line_num + 1);
                        self.has_error_flag = true;
                        return Err(crate::Error::Parse(self.error_message.clone()));
                    }
                }
            ...
            for token in line.split_whitespace() {
                let literal = match token.parse::<i32>() {
                    Ok(literal) => literal,
                    Err(e) => {
                        self.error_message = format!("Invalid literal in line {}: {}", line_num + 1, e);
                        self.has_error_flag = true;
                        return Err(crate::Error::Parse(self.error_message.clone()));
                    }
                };
